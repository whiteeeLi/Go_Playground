...
// receive client关键功能之一，用于接受调用的返回值
...
func NewClient(conn net.Conn, opt *Option) (*Client, error) {
	...
	// 这里需要注意使用的是encoder，所以是将数据编码后发送到conn中，而在server侧使用的是decoder
...
// 这个函数的出现是为了实现option参数的可选化，从感性上讲 ...option的操作其实和切片差不了多少
...
func dialTimeout(f newClientFunc, network, address string, opts ...*Option) (client *Client, err error) {
	...
	// 这里没有使用之前的net.Dial 而是选择了 net.DialTimeout以实现 如果连接创建超时，返回错误。
	...
	// 如果timeout设置为0代表不设置超时检测
	if opt.ConnectTimeout == 0 {
		// 直接在这里阻塞等待
		result := <-ch
		return result.client, result.err
	}
	// 使用select实现多个协程运行结果的处理
	select {
...
func (client *Client) Call(ctx context.Context, serviceMethod string, args, reply interface{}) error {
	...
	select {
	// context包自带超时检测功能，所以从ctx获得信息代表超时了，这个传入的ctx需要用户自己创建并输入
	case <-ctx.Done():
